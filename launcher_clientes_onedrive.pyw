...
GITHUB_EXPANDED_ASSETS_URL = f"{GITHUB_REPO_URL}/releases/expanded_assets/{{tag}}"
GITHUB_LATEST_INSTALLER_URL = f"{GITHUB_REPO_URL}/releases/latest/download/Setup.Buscador.Cliente.HeadCargo.exe"
HOTKEY_ID = 0xA110
MOD_ALT = 0x0001
VK_SPACE = 0x20
WM_HOTKEY = 0x0312
...
def download_text(url: str) -> str:
    ...
    with urllib.request.urlopen(request, timeout=20) as response:
        ...
        if charset in {"utf-8", "utf8"}:
            return raw_content.decode("utf-8-sig", errors="replace").lstrip("\ufeff")
        return raw_content.decode(charset, errors="replace").lstrip("\ufeff")
...
def fetch_latest_release_info() -> tuple[str, str]:
    final_url = resolve_final_url(GITHUB_RELEASES_LATEST_URL)
    parsed = urllib.parse.urlparse(final_url)
    match = re.search(r"/releases/tag/([^/?#]+)", parsed.path)
    if not match:
        raise ValueError("Nao foi possivel identificar a ultima tag de release no GitHub.")

    release_tag = urllib.parse.unquote(match.group(1)).strip()
    latest_version = release_tag[1:] if release_tag.lower().startswith("v") else release_tag
    if not latest_version:
        raise ValueError("A release mais recente do GitHub nao informou a versao.")

    assets_html = download_text(GITHUB_EXPANDED_ASSETS_URL.format(tag=urllib.parse.quote(release_tag)))
    asset_matches = re.findall(r'href="([^"]+/releases/download/[^"]+)"', assets_html, flags=re.IGNORECASE)

    exe_candidates = []
    for asset_path in asset_matches:
        asset_url = urllib.parse.urljoin("https://github.com", asset_path)
        lower_url = urllib.parse.unquote(asset_url).lower()
        if lower_url.endswith(".exe"):
            exe_candidates.append(asset_url)

    prioritized_assets = [
        url for url in exe_candidates
        if "setup" in urllib.parse.unquote(url).lower() and "headcargo" in urllib.parse.unquote(url).lower()
    ]
    installer_url = (prioritized_assets or exe_candidates or [GITHUB_LATEST_INSTALLER_URL])[0]
    return latest_version, installer_url


def fetch_manifest_info() -> tuple[str, str, str]:
    ...
    if not latest_version:
        raise ValueError("Manifesto do GitHub sem 'version'.")
    return latest_version, installer_url, notes
...
class LauncherApp(tk.Tk):
    ...
    def _update_check_worker(self, silent_no_update: bool, silent_errors: bool) -> None:
        try:
            ...
            try:
                release_version, installer_url = fetch_latest_release_info()
            except Exception as exc:
                release_error = str(exc)

            try:
                manifest_version, manifest_installer_url, manifest_notes = fetch_manifest_info()
            except Exception as exc:
            ...
            if release_version:
                ...
                if not installer_url:
                    installer_url = manifest_installer_url or GITHUB_LATEST_INSTALLER_URL
                notes = manifest_notes if manifest_version == release_version else ""
            elif manifest_version:
                latest_version = manifest_version
                installer_url = manifest_installer_url or GITHUB_LATEST_INSTALLER_URL
                notes = manifest_notes
            else:
                errors = [message for message in (release_error, manifest_error) if message]
                raise ValueError(" | ".join(errors) or "Nao foi possivel ler a release nem o manifesto do GitHub.")
            ...
            self.after(0, lambda error_message=error_message: self._handle_update_error(error_message, silent_errors))
    ...
    def _download_and_apply_update_worker(self, installer_url: str, target_version: str) -> None:
        ...
        try:
            ...
            error_message = str(exc)
            self.after(0, lambda error_message=error_message: messagebox.showerror("Falha ao baixar atualizacao", error_message))
    ...
    def _scan_worker(self, root_path: Path) -> None:
        try:
            ...
            error_message = str(exc)
            self.after(0, lambda error_message=error_message: self._finish_scan_error(error_message))
