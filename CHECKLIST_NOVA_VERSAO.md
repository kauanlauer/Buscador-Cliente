...
3. Atualize a versao do executavel do Windows em:
- [buscador_cliente_headcargo_version_info.txt](/C:/Users/Kauan%20Lauer/Documents/Scripts/Buscador/buscador_cliente_headcargo_version_info.txt)
  Procure por `filevers`, `prodvers`, `FileVersion` e `ProductVersion`

4. Gere os executaveis:
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\build_instalador_buscador.ps1
```

5. Atualize o manifesto do GitHub com a nova versao e uma nota curta:
```powershell
...
6. Monte a pasta `Github` com os arquivos que vao para o repositorio:
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\preparar_pasta_github.ps1
```

7. Suba para o repositorio GitHub somente os arquivos que estiverem dentro da pasta `Github`.

8. Crie um novo `Release` no GitHub e anexe este arquivo:
